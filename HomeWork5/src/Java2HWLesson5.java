...
class ArrayComputingCompare implements Runnable {
    ...
    public void runDirect(){
        ...
        for(int i=0;i<arr.length;i++)
            arr[i] = (float)(arr[i] * Math.sin(0.2f + i / 5) * Math.cos(0.2f + i / 5) * Math.cos(0.4f + i / 2));
        System.out.println("Direct run for array size ("+arr.length+") took: "+(System.currentTimeMillis()-a)+" ms");
    ...
    public void run(){
        ...
        System.out.println("Run for "+(arrnum+1)+" part of array size ("+(sparr[arrnum].length)+") took: "+
                (System.currentTimeMillis()-a)+" ms");
